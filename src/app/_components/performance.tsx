"use client"

import { url } from "inspector"
import { ArrowIconGreen, ArrowIconRed } from "./arrowIcon"
import { PerformanceDownIcon } from "./performanceIcons"

const Performance = () => {

    return(
        <>

        <div className="w-full text-[#fff] bg-cover bg-center bg-no-repeat" style={{backgroundImage: "url(/performance-image.webp)"}}> 
            <div className="w-full h-full" style={{background:"linear-gradient(90deg, #00000000 0%, #000000 100%);"}}>             
                <div className="max-w-screen-xl md:flex justify-between mx-auto px-4 items-center mb-7 md:py-60 py-20">
                    <div className="md:w-[37%]">
                        <h2 className="md:text-7xl text-4xl font-medium leading-none mb-7"><strong>Performance</strong><br/>
                            <span className="font-light">Snapshot</span></h2>                                                   
                        <p className="md:mb-0 mb-5">Our recent results highlight strong market performance and operational efficiency</p>  
                    </div>
                    <div className="md:w-[27%] ">
                        <div className="blurBox mb-10 relative py-10 px-5 bg-[#ffffff1a] rounded-[8px] border border-1 border-[#ffffff1a] backdrop-blur-[8px] space-y-5">
                            <h3 className="text-sm mb-5">Properties Leased or Sold Last Quarter</h3>
                            <div className="md:flex gap-5 w-full justify-between">  
                                <div>
                                    <p className="text-[32px] mb-5">448</p>
                                    <span className="flex"><ArrowIconRed /><span className="text-[red] text-[13px]">&nbsp;10% </span>&nbsp;vs last month</span>
                                </div> 
                                <img src="/performance-down.svg" alt="performance" title="performance" className="absolute bottom-10 right-3" />                                
                            </div>
                        </div>
                        <div className="blurBox relative py-10 px-5 bg-[#ffffff1a] rounded-[8px] border border-1 border-[#ffffff1a] backdrop-blur-[8px] space-y-5">
                            <h3 className="text-sm mb-5">Than the Market Average</h3>
                            <div className="md:flex gap-5 w-full justify-between">  
                                <div>
                                    <p className="text-[32px] mb-5">25% Faster</p>
                                    <span className="flex"><ArrowIconGreen /><span className="text-[green] text-[13px]">&nbsp;20% </span>&nbsp;vs last month</span>
                                </div> 
                                <img src="/performance-up.svg" alt="performance" title="performance" className="absolute bottom-10 right-3" />                                
                            </div>
                        </div>
                    </div>            
                </div> 
            </div>           
        </div>
        </>
    )

}

export default Performance